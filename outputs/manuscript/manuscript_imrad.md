# Integrative Multi-Omics and Machine Learning Pipeline for Identification of Novel Drug Candidates Against Multi-Drug Resistant Mycobacterium tuberculosis
...
**Results:** Differential expression analysis identified 426 significantly dysregulated genes (|log₂FC| > 1, adjusted p < 0.05). Pathway enrichment analysis revealed significant perturbation in mycolic acid biosynthesis, oxidative phosphorylation, and cell wall integrity pathways. Multi-factor target scoring across 15 validated TB targets identified InhA as the highest-priority target (composite score incorporating druggability, essentiality, resistance association, expression change, and conservation). Machine learning models achieved ROC-AUC of 0.000 for predicting anti-TB compound activity. Multi-objective ranking of 55 compounds yielded 0 lead candidates with favorable drug-likeness profiles.
...
Molecular descriptors (molecular weight, LogP, TPSA, H-bond donors/acceptors, rotatable bonds) were computed using RDKit where available, or from pre-computed values. Compounds were clustered using K-Means (k=5) on standardized descriptors. Lipinski's Rule of Five was applied for drug-likeness filtering (allowing ≤1 violation).
...
Data were split into training (80%) and test (20%) sets with stratification. Features were standardized using z-score normalization. Model performance was evaluated using accuracy, precision, recall, F1-score, and ROC-AUC. Cross-validation used 5-fold stratified partitioning.
...
Transcriptomic analysis of MDR-TB vs. control samples identified 230 significantly upregulated and 196 significantly downregulated genes (adjusted p < 0.05, |log₂FC| > 1.0). The top differentially expressed gene was 6.0724821475787785. These differentially expressed genes were enriched in critical metabolic and cell wall biosynthesis pathways (Table 1).

**Table 1: Top Enriched Pathways**

The most significantly enriched pathway was mtu00010_Glycolysis, consistent with the known importance of these metabolic processes in Mtb survival and drug resistance mechanisms. Mycolic acid biosynthesis pathway perturbation aligns with the mechanism of action of first-line drugs isoniazid and ethambutol.

### 3.2 Epidemiological Trends

Analysis of WHO TB surveillance data across 10 Indian states (2015-2023) revealed that TB incidence showed an overall Decreasing trend nationally. MDR-TB prevalence exhibited a Rising pattern, with the highest burden in Rajasthan. The national mean MDR-TB prevalence was 8.8%.
...
Multi-factor scoring of 15 validated TB drug targets identified InhA as the highest-priority target with a composite score of 0.870. High-priority targets (score ≥ 0.80) included: InhA, RpoB, GyrA, EmbB, Rrs.
...
Analysis of 55 compounds yielded molecular descriptors across 20 computed features. K-Means clustering identified 5 distinct chemical clusters. Lipinski Rule of Five analysis showed 55 compounds (100.0%) passed the drug-likeness filter (≤1 violation), while 0 compounds failed.

### 3.5 Machine Learning Model Performance

The best-performing model was **N/A** with ROC-AUC of 0.000.
